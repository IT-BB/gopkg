...
import (
	...
	"github.com/gopkg/gopkg/internal/fs"
)

var cmdSave = &Command{
	Usage: "save [-r] [packages]",
	Short: "list and copy dependencies into Gopkgs",
	Long: `
...
The list is written to Gopkgs/Gopkgs.json, and source code for all
dependencies is copied into Gopkgs/_workspace.
...
The dependency list is a JSON document with the following structure:
	...
	type Gopkgs struct {
		...
		Packages   []string // Arguments to gopkg save, if any.
		Deps       []struct {
			ImportPath string
...
To update a dependency to a newer revision, use 'gopkg update'.
...
func save(pkgs []string) error {
	...
	manifest := filepath.Join("Gopkgs", "Gopkgs.json")
	var gold Gopkgs
	oldIsFile, err := readOldGopkgs(&gold)
	if err != nil {
		return err
	}
	gnew := &Gopkgs{
	...
	os.Remove("Gopkgs") // remove regular file if present; ignore error
	readme := filepath.Join("Gopkgs", "Readme")
	...
	//   gopkg go list ./...
	workspace := filepath.Join("Gopkgs", "_workspace")
...
func readOldGopkgs(g *Gopkgs) (isFile bool, err error) {
	f, err := os.Open(filepath.Join("Gopkgs", "Gopkgs.json"))
	if err != nil {
		isFile = true
		f, err = os.Open("Gopkgs")
	}
...
func carryVersions(a, b *Gopkgs) error {
	for i := range b.Deps {
...
func carryVersion(a *Gopkgs, db *Dependency) error {
...
const (
	Readme = `
This directory tree is generated automatically by gopkg.

Please do not edit.

See https://github.com/gopkg/gopkg for more information.
`
	needRestore = `
mismatched versions while migrating

It looks like you are switching from the old Gopkgs format
(from flag -copy=false). The old format is just a file; it
doesn't contain source code. For this migration, gopkg needs
the appropriate version of each dependency to be installed in
GOPATH, so that the source code is available to copy.

To fix this, run 'gopkg restore'.
`
)
