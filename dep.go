...
import (
	...
	"github.com/gopkg/gopkg/internal/vcs"
)

// Gopkgs describes what a package needs to be rebuilt reproducibly.
// It's the same information stored in file Gopkgs.
type Gopkgs struct {
	ImportPath string
	GoVersion  string
...
func (g *Gopkgs) Load(pkgs []*Package) error {
	var err1 error
...
func ReadGopkgs(path string, g *Gopkgs) error {
	f, err := os.Open(path)
	if err != nil {
		return err
	}
...
func copyGopkgs(g *Gopkgs) *Gopkgs {
	h := *g
...
func ReadAndLoadGopkgs(path string) (*Gopkgs, error) {
	g := new(Gopkgs)
	err := ReadGopkgs(path, g)
	if err != nil {
		return nil, err
	}
	err = g.loadGoList()
...
func (g *Gopkgs) loadGoList() error {
...
func (g *Gopkgs) WriteTo(w io.Writer) (int64, error) {
...
//
//   ImportPath                      RepoPath
//   github.com/gopkg/database       $spool/github.com/gopkg/database
//   github.com/gopkg/database/mysql $spool/github.com/gopkg/database
...
func goVersion() (string, error) {
	// Gopkg might have been compiled with a different
